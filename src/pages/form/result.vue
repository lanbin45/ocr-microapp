  ...
  methods: {
    ...
    getOcrResult () {
      ...
      wx.request({
        ...
        data: {
          requestId: $this.requestId, /* '11518496_427914', */
        ...
        success: function (response) {
          ...
          if (resData.error_code) {
            if (Number(resData.error_code) === 18) {
              ...
              setTimeout($this.getOcrResult(), 1000)
            } else {
              wx.showToast({
                title: '邮箱格式错误',
                icon: 'cancel',
    ...
    sendEmail (url) {
      var $this = this
      wx.request({
        url: 'https://alphaconsulting.top/microapp/api/downloadformexcel', // 仅为示例，并非真实的接口地址
        data: {
          fileUrl: url,
          email: this.email
        },
